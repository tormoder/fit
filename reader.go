...
func (d *decoder) parseDefinitionMessage(recordHeader byte) (*defmsg, error) {
	...
	if recordHeader&0x20 != 0 {
		...
		if err = d.readFull(d.tmp[0 : 3*uint16(devFieldsNum)]); err != nil {
			return nil, fmt.Errorf("error parsing fields: %v", err)
		}

		dm.fieldDevDefs = make([]fieldDevDef, devFieldsNum)
		for i, fdd := range dm.fieldDevDefs {
			fdd.fieldNum = d.tmp[i*3]
			fdd.size = d.tmp[(i*3)+1]
			fdd.devDataIndex = d.tmp[(i*3)+2]
...
func (d *decoder) validateFieldDef(gmsgnum MesgNum, dfield fieldDef) error {
	...
	if !pfield.t.Array() {
		...
		case dfield.btype.Size() > pfield.t.BaseType().Size():
			return fmt.Errorf(
		...
		case dfield.btype.Size() <= pfield.t.BaseType().Size() && dfield.btype != pfield.t.BaseType():
...
func (d *decoder) parseDataFields(dm *defmsg, knownMsg bool, msgv reflect.Value) (reflect.Value, error) {
	...
	for i, ddfield := range dm.fieldDevDefs {
		...
		if err != nil {
			return reflect.Value{}, fmt.Errorf(
				"error parsing data developer message: %v (field %d [%v] for [%v])",
				err, i, ddfield, dm)
		}
	}
